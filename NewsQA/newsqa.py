...
class NewsQaModel(object):
    ...
    def evaluate(self, eval_set, feature_idx_map, device, verbose=1):
        ...
        for idx, batch in enumerate(eval_set):
            ...
            with torch.no_grad():
                ...
                input_features = {feature: batch[i].to(device) for feature, i in feature_idx_map.items()}
                # Get the output
                outputs = self.model(**input_features)
    ...
    def load(self, filename):
        ...
        Loads a saved model
        '''
        self.model = torch.load(filename, map_location=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
